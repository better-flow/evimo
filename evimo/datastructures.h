...
public:
    typedef DType value_type;
    typedef DType* ptr_value_type;

    CircularPointerArray () : current_size(0), head_id(0), span_checked(true), latest(NULL) {
        this->data = new ptr_value_type[SZ];
        this->head = this->data;
    }

    ~CircularPointerArray () {
        delete [] this->data;
    }
...
protected:
    class _CAiterator {
    friend class CircularPointerArray;
    public:
        DType& operator *() { return **(this->ptr); }
        DType* operator->() { return *(this->ptr); }
