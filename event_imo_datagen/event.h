...
public:
    ...
    Event () : fr_x(UINT_MAX), fr_y(UINT_MAX), polarity(1), t(ULLONG_MAX), timestamp(LLONG_MAX),
        noise(true), pr_x(NAN), pr_y(NAN), nx(0), ny(0), nz(NZ), u(0), v(0),
        best_u(0), best_v(0), max_score(0), best_pr_x(NAN), best_pr_y(NAN),
        visited(false), cl_id(-1) {}

    Event (uint x_, uint y_, ull t_) : fr_x(x_), fr_y(y_), polarity(1), t(t_), timestamp(t_),
        noise(false), valid(false), pr_x(x_), pr_y(y_), nx(0), ny(0), nz(NZ), u(0), v(0),
        best_u(0), best_v(0), max_score(0), best_pr_x(x_), best_pr_y(y_),
        visited(false), cl_id(-1) {}

    Event (uint x_, uint y_, ull t_, char pol) : fr_x(x_), fr_y(y_), polarity(pol), t(t_), timestamp(t_),
        noise(false), valid(false), pr_x(x_), pr_y(y_), nx(0), ny(0), nz(NZ), u(0), v(0),
        best_u(0), best_v(0), max_score(0), best_pr_x(x_), best_pr_y(y_),
        visited(false), cl_id(-1) {}
    ...
    inline void set_local_time(ull t_) {
        ...
        this->t_sec = (float)(double(this->t) / 1000000000.0);
    }
